...
def test_the_static_selection_keys_match_what_the_builders_emit(seeded):
    ...
    assert pairs == {"feed": "active_thread_id", "rooms": "active_room_id", "dossier": "active_artifact_id"}
    builders = {
...
def test_compute_health_reads_every_input_the_spec_names(needle):
    ...
    launches under it. Input set is sweep §3.7. ``computeHealth`` is kept
    pure over the bundle's ``panels`` so K4 can lift it into
    ``console_render.js`` and the Node harness can run the real matrix
    (sweep test 22); this is the Python stand-in until then."""
    assert needle in _fn_body("computeHealth")
...
def test_the_doctor_run_flow_recomputes_health(seeded):
    ...
    read before the run."""
    handler_start = HTML.index('qs(\'[data-role="doctor-run-btn"]\').addEventListener')
    handler = HTML[handler_start : handler_start + 2000]
