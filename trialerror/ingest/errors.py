...
class QueryEmbedUnavailable(RuntimeError):
    ...
    Defined in this leaf module, and re-exported from
    :mod:`trialerror.ingest.backends` where callers import it from, so that
    :class:`trialerror.retrieve.errors.QueryEmbedBackendUnrunnableError` can
    inherit it without ``retrieve`` importing the whole backends module."""
