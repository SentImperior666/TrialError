...
def _offload_cpu_query_backend(config: dict[str, Any]) -> EmbedBackend:
    """The local CPU query embedder an OFFLOAD program needs.

    The public mirror's own answer to the same problem lane F-1 solved
    (docs/VASTAI_EMBED_DESIGN.md section 6, TRIALERROR_FEEDBACK.md item 15):
    a program whose chunks are embedded on a GPU executor has an
    :class:`OffloadMarker` for its document backend, and that marker raises
    by design -- so query strings, which must be embedded HERE, get a
    :class:`CpuQueryEmbedBackend` running the same ``embed_backend.py`` in a
    short-lived subprocess, its ``model_key``/``dims`` inherited from
    ``[ingest.embed]``. Unconfigured raises :class:`QueryEmbedUnavailable`,
    never the marker."""
    marker = OffloadMarker("embed", config)  # validates model_key / gpu
    query = (config or {}).get(QUERY_EMBED_SUBTABLE_KEY) or {}
    ...
    if not python_exe or not module_dir:
        raise QueryEmbedUnavailable(
            ...
            f"local CPU embedder: set [{QUERY_EMBED_TABLE}] python_exe and module_dir in trialerror.toml to "
...
def load_query_embed_backend(config: dict[str, Any]) -> EmbedBackend:
    ...
    if (
        isinstance(sub, dict)
        and not sub.get("backend")
        and sub.get("python_exe")
        and sub.get("module_dir")
        and (config or {}).get("backend", "fake") == OFFLOAD_BACKEND_NAME
    ):
        # An offload program whose query sub-table is written in the CPU
        # embedder's own shape (``python_exe`` + ``module_dir``, no
        # ``backend`` name): that is the public mirror's configuration and
        # it resolves to :class:`CpuQueryEmbedBackend`. Naming a backend
        # explicitly, or configuring nothing at all, goes through the
        # general resolution below -- where "nothing at all" on an offload
        # program is the document marker, a backend that reports itself
        # NOT runnable rather than raising, which is what the degrade and
        # the doctor check are built on.
        return _offload_cpu_query_backend(config)
