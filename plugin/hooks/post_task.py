#!/usr/bin/env python3
"""Compatibility shim: the post_task hook now lives in
:mod:`trialerror.hooks.post_task` so it can be reached through the
``trialerror`` console script (``trialerror hook post-task``) instead of a
...
from trialerror.hooks.post_task import _evaluate, main  # noqa: F401  (re-exported for callers/tests)
