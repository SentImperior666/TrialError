#!/usr/bin/env python3
"""Compatibility shim: the session_start hook now lives in
:mod:`trialerror.hooks.session_start` so it can be reached through the
``trialerror`` console script (``trialerror hook session-start``) instead of a
...
from trialerror.hooks.session_start import _evaluate, main  # noqa: F401  (re-exported for callers/tests)
