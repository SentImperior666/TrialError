#!/usr/bin/env python3
"""Compatibility shim: the stop_check hook now lives in
:mod:`trialerror.hooks.stop_check` so it can be reached through the
``trialerror`` console script (``trialerror hook stop-check``) instead of a
...
from trialerror.hooks.stop_check import _evaluate, main  # noqa: F401  (re-exported for callers/tests)
