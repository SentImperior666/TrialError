...
Claude Code invokes this script (via ``python plugin/hooks/spawn_gate.py``
 - design Section 12 M6 row: "hook command lines invoke `python` explicitly
(Windows)") for every tool call the plugin's hook configuration matches
...
TRIALERROR-DEV-NOTE (matcher wiring): this script assumes it is only invoked for
``tool_name == "Task"`` - the plugin's hook manifest (``plugin/hooks.json``
or equivalent Claude Code plugin config) is what is supposed to enforce
that via a ``Task``-only matcher; wiring that manifest is plugin-packaging
scope the M3 build brief did not include in its file list (`trialerror/budget/`,
`plugin/hooks/spawn_gate.py` only). Until that manifest exists, THIS script
still defends itself: any ``tool_name`` other than ``"Task"`` (or payload it
can't parse at all) passes through (exit 0) rather than guessing - see
``_evaluate`` below.
